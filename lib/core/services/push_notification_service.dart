...
class PushNotificationService {
  ...
  UnregisterDeviceToken get _unregisterDeviceToken => sl<UnregisterDeviceToken>();

  /// Register device token for the current user
  Future<void> registerDeviceTokenForUser(String userId) async {
    try {
      // Get OneSignal player ID
      final playerId = await OneSignalConfig.getPlayerId();
      if (playerId == null) {
        LoggerService.warning('Cannot register device token: No OneSignal player ID');
        return;
      }
      ...
      LoggerService.error('Error registering device token', e, stackTrace);
    }
  }
  ...
  Future<void> unregisterDeviceToken() async {
    try {
      ...
      final playerId = await OneSignalConfig.getPlayerId();
      if (playerId == null) {
        LoggerService.warning('Cannot unregister device token: No OneSignal player ID');
        return;
      }
